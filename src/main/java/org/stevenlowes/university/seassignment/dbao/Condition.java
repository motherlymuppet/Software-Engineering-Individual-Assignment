...
public class Condition {
    @NotNull
    private static String scriptFolder = "conditions/";
    private final long id;
    @NotNull
    private String name;

    private Condition(long id, @NotNull String name) {
        this.id = id;
        this.name = name;
    }

    public Condition(@NotNull ResultSet rs) throws SQLException {
        this(rs.getLong("conditions.id"), rs.getString("conditions.name"));
    }

    public Condition(long id) throws SQLException {
    ...
    public Condition(@NotNull String name) throws SQLException {
    ...
    public static List<Condition> list() throws SQLException {
        return Tx.execute(conn -> {
            ResultSet rs = Tx.prep(conn, scriptFolder + "list").executeQuery();
            List<Condition> list = new ArrayList<>();
            while (rs.next()) {
                list.add(new Condition(rs));
            }
            return list;
        });
    }
